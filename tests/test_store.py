...
class TestMigrateLegacyDb:
    ...
    def test_migrates_legacy_db_to_new_path(self, tmp_path: Path) -> None:
        ...
        with patch("cq.store._LEGACY_DB_PATH", legacy_db):
    ...
    def test_migration_failure_does_not_crash(self, tmp_path: Path) -> None:
        ...
        with (
            patch("cq.store._LEGACY_DB_PATH", legacy_db),
            patch("cq.store.shutil.move", side_effect=OSError("cross-device link")),
        ):
            _migrate_legacy_db(new_path)
...
class TestCrossLanguageFixtures:
    ...
    def test_legacy_tier_normalized_to_proto(self, legacy_fixture: Path) -> None:
        data = legacy_fixture.read_text()
        unit = KnowledgeUnit.model_validate_json(data)
        # Legacy "team" should be normalized to TIER_PRIVATE.
        assert unit.tier.value.startswith("TIER_")

    def test_legacy_fixture_roundtrips_as_proto(self, legacy_fixture: Path) -> None:
        """Legacy data deserializes and re-serializes in proto-canonical format."""
        data = legacy_fixture.read_text()
        unit = KnowledgeUnit.model_validate_json(data)
        reserialized = unit.model_dump_json()
        restored = KnowledgeUnit.model_validate_json(reserialized)
        assert restored == unit
        # The re-serialized JSON must not contain legacy values.
...
class TestSchemaMigration:
    """Tests for the metadata table and v1 migration."""

    def test_new_store_has_schema_version_1(self, store: LocalStore) -> None:
        row = store._conn.execute("SELECT value FROM metadata WHERE key = 'schema_version'").fetchone()
    ...
    def test_migration_rewrites_legacy_tier(self, tmp_path: Path) -> None:
        """A legacy DB with "team" tier is migrated to "TIER_PRIVATE"."""
        ...
        conn.executescript(
            ...
            CREATE TABLE knowledge_unit_domains (
                unit_id TEXT NOT NULL, domain TEXT NOT NULL,
                FOREIGN KEY (unit_id) REFERENCES knowledge_units(id) ON DELETE CASCADE,
                PRIMARY KEY (unit_id, domain)
            );
            CREATE INDEX idx_domains_domain ON knowledge_unit_domains(domain);
            CREATE VIRTUAL TABLE knowledge_units_fts
                USING fts5(id UNINDEXED, summary, detail, action);
            """
        )
        legacy_data = (_FIXTURES_DIR / "ku_002.json").read_text()
        import json

        ku_id = json.loads(legacy_data)["id"]
        conn.execute(
        ...
        assert unit.tier == Tier.PRIVATE
        assert unit.tier.value == "TIER_PRIVATE"

        # Backup was created.
        assert Path(str(db_path) + ".pre-migration-bak").exists()
        s.close()
    ...
    def test_migration_is_idempotent(self, tmp_path: Path) -> None:
        ...
        s1.close()
        # Re-opening should not crash or re-migrate.
        s2 = LocalStore(db_path=db_path)
        assert s2._schema_version() == 1
        s2.close()
