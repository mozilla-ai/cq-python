...
class LocalStore:
    ...
    def _set_schema_version(self, version: int) -> None:
        """Write the schema version to the metadata table."""
        self._conn.execute(
    ...
    def _migrate_to_v1(self) -> None:
        """Rewrite legacy enum values to proto-canonical format.

        Pre-proto code stored tiers as "local"/"team"/"global" and flag
        reasons as "stale"/"incorrect"/"duplicate". This migration rewrites
        all JSON blobs to use the proto-canonical values so both Go and
        Python SDKs read the same format.
        """
        rows = self._conn.execute("SELECT id, data FROM knowledge_units").fetchall()
        if not rows:
            self._set_schema_version(1)
            return

        # Checkpoint WAL so backup is self-contained.
        self._conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")
        bak_path = str(self._db_path) + ".pre-migration-bak"
        shutil.copy2(str(self._db_path), bak_path)
        logger.info("Database backed up to %s", bak_path)

        migrated = 0
        for unit_id, data in rows:
            ku = KnowledgeUnit.model_validate_json(data)
            new_data = ku.model_dump_json()
            if new_data != data:
                self._conn.execute(
        ...
        self._set_schema_version(1)
        logger.info("Migration complete: %d of %d units rewritten.", migrated, len(rows))
    ...
    def insert(self, unit: KnowledgeUnit) -> None:
        ...
        """
        domains = _normalize_domains(unit.domain)
        if not domains:
